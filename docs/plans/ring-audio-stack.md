...
| **TALK on the production stack, real listener** | ✅ **verified 2026-08-26**: `mic:` + `ring:` deployed to production go2rtc (E9 row above), the owner held the Panel popup's talk button and a person at the door heard them. Inbound (hearing the door on the Panel) not yet assessed — the owner's machine had a local sound issue during the test |
| **LISTEN in the Panel (inbound audio at the wall)** | ✅ **implemented 2026-08-26** — not via §1's gst-launch leg but through the Panel's new RTSP/fvp video transport (phase-8 N5/N12): the doorbell Popup unmutes its own session (`LiveVideoSession.setMuted`, born-muted seam-wide) and **ducks while the talk button is held** — ADR-0011's half-duplex mechanism. Caveats: RTSP transport only (a `VIDEO_TRANSPORT=mjpeg` rollback trades inbound audio away), and unverified by ear pending the owner's machine sound fix. §1's gst pipeline stays the appliance-provisioning reference |
