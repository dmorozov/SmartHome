...
> **Newer series:** [`sh3d-import/`](sh3d-import/README.md) (2026-08-01, written against `d01f290`) — five phases making the Sweet Home 3D drawing the source of Device placements, derived from the verified critical analysis of `sh3d-dollhouse-spec-final.md`. **Phases 0, 1 and 2 have landed** (2026-08-02); 3–4 remain. It **supersedes plan 06 in part** (see phase 3) and interacts with plans 07/08 only through `ha_hub.dart`/`main.dart` file-level merges (see its README's coordination section).
>
...
| [06](06-device-vocabulary-table.md) | Device vocabulary table + shared devices.yaml parser — **SUPERSEDED IN PART; implement [`sh3d-import/phase-3`](sh3d-import/phase-3-device-vocabulary.md) instead.** The scheduling and conflict rows below still name plan 06 as written; read them as applying to phase 3, which inherits its five surviving deliverables | ~~Worth exploring~~ | Kind → slug/state-family/seed in one pure-Dart table; the two divergent devices.yaml parsers collapse into one; the dev-entity generator becomes testable | new `domain/device_vocabulary.dart`, `data/device_parser.dart`, `tool/dev_entities_core.dart`; `house.dart`, `house_loader.dart`, `fake_hub.dart`, `ha_hub.dart`, `tool/gen_dev_entities.dart` |
