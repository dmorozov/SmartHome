...
void main() {
  ...
  group('the probe grid (CAMERAS_GRID=probe)', () {
    // Bisect scaffolding is pinned like everything else — an arm that
    // quietly dials the wrong plumbing sends the whole hunt down the wrong
    // branch, which this session has already paid for twice.
    Device probeCam(
      String id, {
      String? stream,
      String? sub,
    ...
    Future<void> pumpProbe(
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: CamerasProbeGrid(
              devices: devices,
              // The trap is the point: the wall's VideoConfig.open is the
              // keep-alive pool, and bypassing it is this arm's reason to
              // exist. Only urlFor may be consulted.
              video: VideoConfig(
                go2rtcUrl: 'http://hub:1984',
                open: (url, {required name}) => fail(
                  'the probe grid dialled VideoConfig.open — the plumbing '
                  'it exists to bypass',
                ),
              ),
              opener: go2rtc.open,
    ...
    testWidgets(
      'dials every camera stream, substream first — and the doorbell '
      'never, whatever its bindings say (#177014)',
      (tester) async {
        final go2rtc = FakeGo2rtc();
        await pumpProbe(tester, go2rtc, [
          probeCam('back', stream: 'wyze_back', sub: 'wyze_back_sub'),
          probeCam('garage', stream: 'wyze_garage'),
          probeCam('ring', kind: DeviceKind.doorbell, stream: 'ring_doorbell'),
          probeCam('bare'),
        ]);
        await tester.pump(const Duration(seconds: 5));
        expect(
          [for (final s in go2rtc.opened) s.name],
          ['wyze_back_sub', 'wyze_garage'],
        );
        expect(
          go2rtc.opened.first.url.queryParameters['src'],
          'wyze_back_sub',
          reason: 'the endpoint is VideoConfig.urlFor over the real go2rtc',
        );
        await tester.pumpWidget(const SizedBox.shrink());
      },
    );

    testWidgets('dials are staggered, one per second — the admission '
        "doctrine a debug arm doesn't get to ignore", (tester) async {
      final go2rtc = FakeGo2rtc();
      await pumpProbe(tester, go2rtc, [
        probeCam('a', stream: 'cam_a'),
        probeCam('b', stream: 'cam_b'),
        probeCam('c', stream: 'cam_c'),
      ]);
      expect(go2rtc.opened, hasLength(1));
      await tester.pump(const Duration(seconds: 1));
      expect(go2rtc.opened, hasLength(2));
      await tester.pump(const Duration(seconds: 1));
      expect(go2rtc.opened, hasLength(3));
      await tester.pumpWidget(const SizedBox.shrink());
    });

    testWidgets('every session renders and unmounting closes them all — '
        'the view keeps its one promise', (tester) async {
      final go2rtc = FakeGo2rtc();
      await pumpProbe(tester, go2rtc, [
        probeCam('a', stream: 'cam_a'),
        probeCam('b', stream: 'cam_b'),
      ...
      await tester.pump(const Duration(seconds: 1));
      expect(find.text('a moving picture'), findsNWidgets(2));

      await tester.pumpWidget(const SizedBox.shrink());
      expect([for (final s in go2rtc.opened) s.closes], [1, 1]);
    });
  });
}
