...
def main():
    ...
    if not raw_rooms:
        ...
        report_and_exit(errors, warnings)
    ...
    name = args.name or root.get('name') or 'House'
    name = re.sub(r'\.sh3d\s*$', '', name).strip()
    out = [f'# Generated by sh3d_to_yaml.py from {args.input} — '
