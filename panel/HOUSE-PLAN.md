...
| `only a doorbell has a speaker to talk out of` | you put `talk:` on a camera or anything else; only a doorbell can be spoken through, so delete the line or fix the marker's kind |
...
- **`only a camera or a doorbell plays video` does not repeat the stream
  ...
  `cameras.popup_open` every time the popup opens, and has to.
