...
| 5 | [05-devices-cloud.md](commissioning/05-devices-cloud.md) | Ring, Wyze, HACS, LG, Whisker, Petlibro, Emporia — every vendor-account device | **AS-BUILT** for Ring (§1), HACS (§3), and Emporia (§7). **INSTRUCTIONS** for the rest — Wyze, LG, Whisker, Petlibro are not set up |
...
| **Ring email + password + a live 2FA code** — authenticated 2026-08-05; **15 entities** on device "Front Door", live MJPEG pulled end to end through go2rtc | [5 §1.1](commissioning/05-devices-cloud.md) |
| **A GitHub account** — HACS installed and device-code-authenticated 2026-08-07; entry `01KZDDD6TH4CKTE9FZT1XH102B`, `state: loaded`, `update.hacs_update` entity present | [5 §3.1–3.2](commissioning/05-devices-cloud.md) |
| **Emporia app email + password** — the cloud entry, added 2026-08-08; entry `01KZFNVXV01M73F63FS8EQ19S4`, `state: loaded`, 58 entities across 19 devices (18 Vue 3 CT channels, 1 smart plug) | [5 §7](commissioning/05-devices-cloud.md) |
...
| **HA answers cross-origin browser requests from any origin** (`http: cors_allowed_origins: ["*"]`), paired with `login_attempts_threshold: 5` | Owner, 2026-08-06. Committed to `hub/ha-config/configuration.yaml`; the web Panel cannot fetch a camera still without it | **NOT IN EFFECT ON THIS HUB.** HA reads `configuration.yaml` only at boot and this container has run since 2026-08-05 — verified 2026-08-07, the live Hub still answers a preflight `403`. It needs one HA restart, which is an owner action. The threshold is the load-bearing half: `"*"` makes HA's *unauthenticated* login flow cross-origin readable, and HA ships with no rate limit at all. [3 §3.10](commissioning/03-home-assistant.md) has the measurements and the narrower alternative |
...
| Cloud fleet | **Ring** authenticated 2026-08-05 (15 entities, live video proven). **HACS** installed and authenticated 2026-08-07 (entry `01KZDDD6TH4CKTE9FZT1XH102B`). **Emporia** authenticated 2026-08-08 (entry `01KZFNVXV01M73F63FS8EQ19S4`, 58 entities — whole-house total identified, per-circuit binding waits on the owner naming the other 15 channels in the Emporia app) | Wyze identification is done (**B3**) but flashing + streams are not; LG ThinQ, Whisker, Petlibro are untouched. SmartThings is a decision (D3), not a task |
...
| 8 | **Environment beats `--dart-define`** | The Panel resolves `HUB`/`HA_URL`/`HA_TOKEN`/`GO2RTC_URL`/`LOG` **environment first**. That is why every `panel_*` var that carries a Panel **setting** — the ones that become `Environment=` lines — defaults to `""` in `group_vars/all.yml` and must stay empty (`panel_env_file` is a path and `panel_ha_token` travels by file, so neither is one of these) — a "harmless" default there silently forces a Panel built with `--dart-define=HUB=ha` back onto the fake Hub. And **web builds have no process environment**: on `-d chrome` an exported `HA_URL` is discarded and the boot log says `env=unavailable` | [6 §6.5, §6.7](commissioning/06-panel-and-bindings.md) |
